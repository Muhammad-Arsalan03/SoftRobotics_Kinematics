...
%n = 29; % number of sections of the Soft Growing Robot
...
%%
  ...
  for n=30:-1:1
    motion1 = 1:180; % input displacment of the tendon
...
motion2 = 1:170; % input displacment of the tendon
...
%     daspect([1 1 1])
%     grid minor
%     print('SGR_6stages_trajectories with decreasing sections','-dtiff','-r300')
%     
%     
...
%     %%
%     figure (2)
%     plot(q1(n+1,1:(2*interval):end),q1(n+1,2:(2*interval):end),'.-');%interval is used to adjust the motion finess to plot the robot's shape
%     hold on
%     plot(q2(n+1,1:(2*interval):end),q2(n+1,2:(2*interval):end),'.-');
%     xlabel('X coordinate(mm)');
%     ylabel('Y cooridnate(mm)');
%     % axis([0 ((ls+0.5*lg)+(n-1)*(ls+lg))+0.5*(ls+lg) 0 50]);
%     axis([-300 600 0 400])
%     daspect([1 1 1])
%     grid minor
%     print('SGR_6stages_workspace with decreasing sections','-dtiff','-r300')
%     
%     
    % %%
    % % %%%%%%%%% plot of SGR bending %%%%%%%%%%%%%%
    figure (3)
    myVideo = VideoWriter('SGR_Bending_Demonstration_6stages with decreasing sections'); %open video file
    ...
    for i=1:size(q1,2)/2
        ...
        y1=q1(:,2*i); 
        robot1=plot(x1,y1,'.-r');
        w1=plot(q1(n+1,1:(2*interval):end),q1(n+1,2:(2*interval):end),'.-');
        hold on;
    ...
    for i=1:size(q2,2)/2
        ...
        y2=q2(:,2*i); 
        robot2=plot(x2,y2,'.-r');
        w2=plot(q2(n+1,1:(2*interval):end),q2(n+1,2:(2*interval):end),'.-');
        hold on;
        axis([0 600 0 400]) % axis limitation
        daspect([1 1 1])
        xlabel('X coordinate(mm)');
